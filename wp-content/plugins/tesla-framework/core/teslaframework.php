...
if (!class_exists('TeslaFrameworkPL')){
	class TeslaFrameworkPL {
		...
		function __construct(){
			...
			add_action('after_setup_theme',array($this,'metaboxes'));
			$this->load_scripts();
			$this->theme_domain();
			add_action('after_setup_theme',array($this,'require_widgets')); //after_setup_theme used for locate_template to be available
		}
		...
		function metaboxes(){
			...
			*/
			...
			if (!empty($meta_options)){
				...
				require_once(TT_FW_PL . "extensions/meta-box-class/my-meta-box-class.php");
				require_once(TT_FW_PL . "extensions/Tax-meta-class/Tax-meta-class.php");
				$prefix = THEME_NAME . "_";
		...
		function function_checks(){
			...
			if (file_exists(TT_FW_PL . 'core/tt_security.php')){
				if (strlen(trim(preg_replace('/\s\s+/', ' ', file_get_contents(TT_FW_PL . 'core/tt_security.php')))) !== 4277 )
					return FALSE;
			}else
				return FALSE;
			return TRUE;
		...
		function require_widgets(){
			...
			*	@since 1.9.2 
			*/
			if(file_exists(TT_FW_PL . 'widgets')){
				$fw_widgets_files = $this->scan_directory(TT_FW_PL . 'widgets','php');
				foreach ($fw_widgets_files as $fw_widget) {
					if( !locate_template( ltrim( $fw_widget, '/' ) , true, true ) ) {
						require $fw_widget;
					}
				}
			}
