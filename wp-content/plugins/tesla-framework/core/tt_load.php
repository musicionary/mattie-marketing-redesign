    ...
    class TT_Load {
        ...
        function helper( $helper ) {
            if ( $this->helper_exists( $helper ) ) {
                require_once TT_FW_PL . 'helpers/' . $helper . '.php';
            }
            else
        ...
        function helper_exists( $helper ) {
            if ( file_exists( TT_FW_PL . 'helpers/' . $helper . '.php' ) )
                return TRUE;
            else
                return FALSE;
        }
        ...
        function view_exists( $name, $config = NULL ) {
            if($config === NULL)
                $result = file_exists( TT_FW_PL . 'views/' . $name . '.php' );
            else
        ...
        function view( $_name, $_data = NULL, $__return = FALSE, $config = NULL ) {
            ...
            ob_start();
            if($config === NULL)
                require TT_FW_PL . 'views/'.$_name.'.php';
            else
