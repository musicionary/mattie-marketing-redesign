	...
	class TT_Contact_Form_Builder {
		...
		private static function enqueue_frontend_scripts(){
			...
			if($parsley_locale_format !== 'en'){
				if(file_exists(TT_FW_PL . 'static/js/parsley/locales/' . $parsley_locale_format . '.js'))
				...
				if(file_exists(TT_FW_PL . 'static/js/parsley/locales/' . $parsley_locale_format . '.extra.js'))
